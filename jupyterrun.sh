...
# running Qwen vllm , change with any other model
model='ISTA-DASLab/Mistral-Small-3.1-24B-Instruct-2503-GPTQ-4b-128g'

docker run -d --rm --name vllm --runtime nvidia --gpus all     -v ~/.cache/huggingface:/root/.cache/huggingface     --env "HUGGING_FACE_HUB_TOKEN=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"     -p 8000:8000     --ipc=host     vllm/vllm-openai:latest     --model $model    --dtype auto     --api-key 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

docker run -d --rm --memory 10g --memory-swap 40g --shm-size 10G -v /moataz-work:/workspace/moataz-work --name torchjup -p 8888:8888 -p 9000:9000 torchadvrag jupyter-notebook --NotebookApp.token='tmatem'
...
docker run --rm -p 4043:4040 -d -it  --name thenanorok1 -e NGROK_AUTHTOKEN=326cdAM6MZfbaqBRK15aqQUtOg4_4d5KfcAQ3xLvhz2CL9wfq ngrok/ngrok:latest http http://192.168.8.155 --url=definitely-prompt-gibbon.ngrok-free.app
